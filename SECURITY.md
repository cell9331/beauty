...
- Canonical binding, contribution values, unit authorization, and ownership reduction remain package-private, non-Codable, request-local, non-persistent, non-networked, and absent from logs and public results. The engine retains none of them across success, throw, reset, or later requests.
...
- The fail-closed checker passes 44/44 adversarial mutations and names T-55-01 through T-55-07 as seven machine-green HIGH mitigations. Exact-empty admission plus 59/5/72 compatibility and full SwiftPM/Demo regressions block candidate, provider, renderer, preset, Demo, realtime, model, network, resource, target, or dependency drift.
