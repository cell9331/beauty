...
- The exact Git boundary `example-images/local-retouch-review/` is ignored and must remain untracked. D-15/D-16 durable output is constructed by positive allowlist from feature decisions, aggregate counts, and (only for a ready bundle) per-fixture opaque ID/feature/polarity plus the seven structured review judgments. It excludes media, filenames/paths, rights records, reviewer identity, timestamps, session/event metadata, notes/freeform text, raw errors, and geometry; the current empty ledger therefore serializes no review rows but the serializer remains explicitly review-capable.
- The canonical `.planning/phases/54-rights-approved-evidence-and-eligibility-decisions/54-THREAT-INVENTORY.json` is the single Phase 54 HIGH inventory: active T-54-01 through T-54-08, including T-54-07 evidence repudiation and T-54-08 local-review disclosure. The checker derives the HIGH denominator from this inventory and rejects missing, duplicate, extra, malformed, or unrecorded retired/merged rows. Phase 54 adds no SDK or Demo import of the review core, no provider/renderer/preset/admission route, no cloud/model/dependency, and no change to Phase 53 exact-empty production admission.
