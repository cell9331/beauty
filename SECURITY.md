...
- Vision contour and median inputs are untrusted biometric-adjacent data. Each optional region is rejected before provider access when empty, above its 32/16-point ceiling, non-finite, outside closed unit bounds, duplicate, undersized, direction-degenerate, side-inconsistent, or cross-support-inconsistent. A malformed region cannot authorize another region or erase safe shipped siblings.
