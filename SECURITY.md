...
- T-57-01 through T-57-08 are eight machine-green HIGH mitigations. The 490-case live-fixture matrix recursively scans every production Swift file plus package/resource/Demo owners, rejects unsuffixed canonical/synonym aliases, neutral-file and inert routes, recursive Demo ID/title mappings, Chinese/ID proxy coupling, contradictory or sensitive evidence, scanner failures, and stale/duplicate root owners, while clean proxy-only shipped domains and the two exact disabled taxonomy rows remain required controls.
