...
3. Create a private request context. Do not add public fields for landmarks or
   masks.
4. Invoke teeth and sclera providers independently. A failure returns an empty
   local mask for that region; it must not fail or reuse stale support globally.
5. Intersect masks with their anatomical containment. Keep them disjoint where
   expected and build a union only for measurement/compositing.
...
`sources/004-local-color-retouch/`, `sources/005-still-image-integration/`, and
