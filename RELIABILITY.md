...
- Fresh nose work uses the provider's per-field emission contract at the requested effective strength before conflict accounting. Each non-emitting legacy helper is zeroed independently; root/tip validation also includes actual strength/displacement guards, so structurally present but non-emitting work cannot affect totals, scale, or weakened count while emitting sibling nose work continues. Safe face-agnostic domains continue.
