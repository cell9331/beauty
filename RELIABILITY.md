...
- Orientation, input mirroring, preview mirroring, and forward/reversed source order are deterministic because side centroids and endpoint order use mapper-derived face-local axes and canonicalization performs whole-array reversal only. Adapter validation then accepts 4...16 exact-bit-unique finite closed-unit points, chord 0.08...0.50, vertical span at most 0.25, no non-adjacent intersections, and projection epsilon 0.000001.
...
- Evidence is fail-closed: a skipped command, tool error, unclassified checker match, failed test, unresolved ASVS L1 HIGH finding, or missing/unreadable/empty/non-regular `example-images/input/portraits/e1.png` is not success. Full SwiftPM may be called green only after fixture preflight succeeds. Provider/output/cap/promotion, Demo/device, commercial, performance, packaging, shipping, and release-readiness evidence remains Phase 50-52 or later work.
