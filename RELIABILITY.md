...
- Both-direction valid-invalid-valid sequences recover on the final valid request. Sequential and concurrent completion-order calls preserve request identity. Cancelling a task that is already inside the synchronous resolver does not abort that call; the completed result remains request-local and cannot alter a sibling or later rerun. Whether that completed value replaces host state is owned by the host's asynchronous generation/publication policy, not by this synchronous SDK contract.
