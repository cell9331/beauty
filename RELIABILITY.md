...
- Fresh nose work uses the provider's per-field emission contract before conflict accounting. Each non-emitting legacy helper is zeroed independently; package-internal explicit `noseRoot` and `noseTip` supports fail closed independently, so invalid root or tip work cannot mask or remove emitting sibling nose work. Safe face-agnostic domains continue.
