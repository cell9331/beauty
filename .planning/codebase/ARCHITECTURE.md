...
| `BeautyRender` | Render-pass/pixel-buffer foundations plus one byte-pinned inactive shader resource; this is not a GPU execution claim. |
...
Realtime landmark tracking, local teeth/sclera work, and GPU routing are absent.
...
- v1.16 does not modify the retained shader, add a Metal/GPU backend, or claim
  device, commercial, packaging, shipping, launch, or release readiness.
