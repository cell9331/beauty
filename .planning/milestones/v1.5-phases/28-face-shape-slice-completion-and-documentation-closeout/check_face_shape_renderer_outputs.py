...
def top_region_differs(
    ...
    geometry_channels = 4 if geometry.color_type == 6 else 3
    baseline_row_length = baseline.width * baseline_channels
    geometry_row_length = geometry.width * geometry_channels
    baseline_previous = bytearray(baseline_row_length)
    geometry_previous = bytearray(geometry_row_length)
    baseline_offset = 0
    geometry_offset = 0
    ...
    for _ in range(comparable_rows):
        ...
        geometry_filter = geometry.raw[geometry_offset]
        geometry_offset += 1
        geometry_row = bytearray(geometry.raw[geometry_offset : geometry_offset + geometry_row_length])
        geometry_offset += geometry_row_length
        unfilter_scanline(
            geometry_row,
            geometry_previous,
            geometry_channels,
            geometry_filter,
            f"output/{geometry_path.name}",
        )
        ...
        geometry_previous = geometry_row
...
def rows_differ(
    ...
    geometry_row: bytearray,
    geometry_color_type: int,
    width: int,
) -> bool:
    if baseline_color_type == geometry_color_type:
        return baseline_row != geometry_row

    for column in range(width):
        baseline_source = column * (4 if baseline_color_type == 6 else 3)
        geometry_source = column * (4 if geometry_color_type == 6 else 3)
        baseline_pixel = baseline_row[baseline_source : baseline_source + 3]
        geometry_pixel = geometry_row[geometry_source : geometry_source + 3]
        if baseline_pixel != geometry_pixel:
            return True
        baseline_alpha = baseline_row[baseline_source + 3] if baseline_color_type == 6 else 255
        geometry_alpha = geometry_row[geometry_source + 3] if geometry_color_type == 6 else 255
        if baseline_alpha != geometry_alpha:
            return True
    return False


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate generated Phase 28 face-shape PNG outputs.")
...
def main() -> int:
    ...
    expected_comparisons = len(PORTRAIT_FIXTURE_NAMES) * len(PHASE28_GEOMETRY_CASE_IDS)
    if portrait_comparisons != expected_comparisons:
        failures.append(
            f"portrait face-shape-vs-baseline top-region comparisons: "
            f"{portrait_comparisons}/{expected_comparisons}"
        )

    no_face_output = expected_output_name(NO_FACE_FIXTURE_NAME, "jawSlim_0p35")
    ...
    print("phase 28 cases: " + ", ".join(PHASE28_GEOMETRY_CASE_IDS))
    return 0
