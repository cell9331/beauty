...
**Spikes processed:** 10 total; 3 added in this append
...
- The shared still-image pattern remains: one Vision request, private
  request-local support/masks, independent regional failure, bounded transforms
  composed once, aggregate-only events, and no camera/pixel-buffer inference.
...
- the mandatory sclera jitter guard and color-independent safety oracle;
- the still-image integration/privacy boundary; and
- the rights-approved local fixture manifest, blind-review, and sanitized-export
  gate required before product planning.

Exact source for Spikes 006/009/010 and the current 13-test Swift harness is
