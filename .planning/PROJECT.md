...
**Implementation state:** v1.16 froze the current CPU renderer as the reference backend, added generated CPU oracles, a public-only SwiftPM consumer, the SDK-owned renderer CLI, and conditional `BeautyResult` sendability. Phase 70 adds a package-only backend-neutral execution boundary and routes the retained CPU implementation through it, while preserving the 61-field/five-preset/74-case compatibility surface. Later v1.17 phases own Metal behavior; historical application taxonomy remains archive-only.

**Verification state:** Phase 70 passes its backend mutation preflight, focused 11-test contract/CPU/routing coverage, generated CPU preflight with 41 tests, and the archive-first no-skip wrapper with 8 opt-ins, zero skips, and zero failures. Later v1.17 phases must add public backend selection, Metal capability/error, shader/pass, CPU/GPU parity, and compatibility evidence without weakening the existing CPU oracle or silently falling back. Device/performance/commercial/packaging/shipping/launch/release-readiness remain separate scopes.
