...
**Verification state:** Combined public output byte-matches independently merged standalone outputs; collisions preserve source and injected failure units retain unaffected bytes. Named-sRGB facade carriers and saved PNGs, request-local privacy, 61/5/74 compatibility, and both private 6/6 output matrices remain the bounded SDK-core baseline. The Phase 67 public-only consumer, 24-case renderer regression, and 7-case compiled Process matrix pass before the mandatory archive → boundary → consumer → one-child SwiftPM conjunction. The measured full child executes 658 tests with zero failures and zero skips, including all eight opt-ins. Device/performance/commercial/packaging/shipping/launch/release-readiness remain separate scopes.
...
**Code size:** the immutable `v1.15` tag contains 49,018 tracked Swift source/test lines. The active post-archive SDK-only tree contains 66 Swift source files / 14,950 source lines and 51 SwiftPM test files / 28,093 test lines, excluding `.build` and archive contents.
...
**Status:** Completed, independently audited, and archived on 2026-08-11. This lifecycle completion is not a distribution, shipping, launch, commercial-quality, or release-readiness claim.
...
**Key context:** v1.15 is SDK-core and opaque still-image only. Teeth and sclera each passed separate rights-approved positive/negative evidence, provider, output, safety, review and promotion chains, followed by combined failure/privacy/compatibility closeout. Transparent input remains rejected. No realtime/pixel-buffer local-retouch path, SwiftUI/Demo activation, network/cloud behavior, third-party beauty SDK, unapproved model or weights, public raw landmarks/masks, tracked fixture media, commercial/device/performance/packaging/shipping/launch/release-readiness claim, or `去脂` proxy is included.
...
| v1.15 completes `白牙` before independently completing `祛红血丝`, while deferring `去脂`. | The user wants two finished visible effects one at a time; their evidence, anatomy, safety oracles, and failure units differ, and upper-eyelid fullness still lacks a credible approved non-warp path. | Verified: `白牙` completed in Phase 61, `祛红血丝` completed after independent Phases 62-64, and Phase 65 closed combined behavior without admitting `去脂`. |
| A local candidate authorization does not open a v1.15 feature gate. | `portrait_002` is C2PA-declared AI-generated and remains mechanics-only even if feature-specific derivatives are completed; candidate status cannot replace either feature's rights-approved genuine positive/negative bundle. | Verified for teeth and retained for sclera; the passed teeth chain uses a separate genuine pair |
...
| Product remains an SDK, not a standalone consumer app. | The SDK facade, algorithms, render pipeline, and SwiftPM consumer behavior are the product. | Reaffirmed for v1.16/v1.17; Demo becomes a historical archive. |
| Demo should become a rich Meitu/Xingtu-style showcase. | v1 validated a broad Demo surface without making the Demo the primary product. | Historical; superseded on 2026-08-13 by the SDK-only archive direction. |
| Demo uses modules only through the `BeautySDK` facade. | Facade-only Demo imports kept host integration realistic while the Demo was active. | Historical invariant preserved in the archive; clean SwiftPM consumer coverage replaces it in v1.16. |
...
| Complete the remaining narrow facial-feature taxonomy across v1.13-v1.16, beginning with eyebrows. | Eyebrow controls reuse Apple Vision landmark and unified-warp infrastructure; local retouch follows next, while hairline/double-chin work remains gated on an approved local semantic-region resource. | v1.15 teeth and sclera are complete; `去脂`, hairline, double-chin, and any later taxonomy closeout remain fresh-milestone decisions. |
