#!/usr/bin/env python3
"""Validate the Phase 39 discovered renderer matrix and mouth-local evidence."""
...
MOUTH_Y_PLUS_CASE_ID = "mouthYPosition_plus0p25"
MOUTH_Y_MINUS_CASE_ID = "mouthYPosition_minus0p25"
MOUTH_TILT_PLUS_CASE_ID = "mouthTilt_plus0p25"
MOUTH_TILT_MINUS_CASE_ID = "mouthTilt_minus0p25"
MOUTH_X_PLUS_CASE_ID = "mouthXPosition_plus0p25"
MOUTH_X_MINUS_CASE_ID = "mouthXPosition_minus0p25"
LIP_PEAK_CASE_ID = "lipPeakDefinition_0p25"
LIP_PLUMP_CASE_ID = "lipPlump_0p25"
SMILE_CASE_ID = "smile_0p50"
MOUTH_SIZE_CASE_ID = "mouthSize_plus0p35"
LIP_COLOR_CASE_ID = "lipColor_0p50"
NEW_CASE_IDS = (
    MOUTH_Y_PLUS_CASE_ID,
    MOUTH_Y_MINUS_CASE_ID,
    MOUTH_TILT_PLUS_CASE_ID,
    MOUTH_TILT_MINUS_CASE_ID,
    MOUTH_X_PLUS_CASE_ID,
    MOUTH_X_MINUS_CASE_ID,
    LIP_PEAK_CASE_ID,
    LIP_PLUMP_CASE_ID,
)
REQUIRED_CASE_IDS = {
...
# One global, top-origin normalized mouth rectangle. These are committed inputs,
...
FAMILIES = (
    Family("visibility", "mouth_y_plus_vs_baseline", MOUTH_Y_PLUS_CASE_ID, BASELINE_CASE_ID),
    Family("visibility", "mouth_y_minus_vs_baseline", MOUTH_Y_MINUS_CASE_ID, BASELINE_CASE_ID),
    Family("visibility", "mouth_tilt_plus_vs_baseline", MOUTH_TILT_PLUS_CASE_ID, BASELINE_CASE_ID),
    Family("visibility", "mouth_tilt_minus_vs_baseline", MOUTH_TILT_MINUS_CASE_ID, BASELINE_CASE_ID),
    Family("visibility", "mouth_x_plus_vs_baseline", MOUTH_X_PLUS_CASE_ID, BASELINE_CASE_ID),
    Family("visibility", "mouth_x_minus_vs_baseline", MOUTH_X_MINUS_CASE_ID, BASELINE_CASE_ID),
    Family("visibility", "lip_peak_vs_baseline", LIP_PEAK_CASE_ID, BASELINE_CASE_ID),
    Family("visibility", "lip_plump_vs_baseline", LIP_PLUMP_CASE_ID, BASELINE_CASE_ID),
    Family("signed direction", "mouth_y_plus_vs_minus", MOUTH_Y_PLUS_CASE_ID, MOUTH_Y_MINUS_CASE_ID),
    Family("signed direction", "mouth_tilt_plus_vs_minus", MOUTH_TILT_PLUS_CASE_ID, MOUTH_TILT_MINUS_CASE_ID),
    Family("signed direction", "mouth_x_plus_vs_minus", MOUTH_X_PLUS_CASE_ID, MOUTH_X_MINUS_CASE_ID),
    Family("peak independence", "lip_peak_vs_smile", LIP_PEAK_CASE_ID, SMILE_CASE_ID),
    Family("peak independence", "lip_peak_vs_mouth_size", LIP_PEAK_CASE_ID, MOUTH_SIZE_CASE_ID),
    Family("plump independence", "lip_plump_vs_mouth_size", LIP_PLUMP_CASE_ID, MOUTH_SIZE_CASE_ID),
    Family("plump independence", "lip_plump_vs_lip_color", LIP_PLUMP_CASE_ID, LIP_COLOR_CASE_ID),
    Family("plump independence", "lip_plump_vs_lip_peak", LIP_PLUMP_CASE_ID, LIP_PEAK_CASE_ID),
)

EXPECTED_GROUP_COMPARISONS = {
    "visibility": 48,
    "signed direction": 18,
    "peak independence": 12,
    "plump independence": 18,
}
...
def mouth_roi(width: int, height: int) -> tuple[int, int, int, int]:
    ...
    if right <= left or bottom <= top:
        raise RendererOutputError(f"invalid mouth ROI for {width}x{height}")
    if bottom > comparable_rows:
        raise RendererOutputError(
            f"mouth ROI bottom {bottom} is not wholly above watermark boundary {comparable_rows} for {width}x{height}"
        )
...
def region_difference(reference_path: Path, candidate_path: Path) -> ComparisonMetrics:
    ...
    left, right, top, bottom = mouth_roi(width, height)
    changed = delta = 0
...
def validate_outputs(input_dir: Path, output_dir: Path, renderer_source: Path, measure: bool) -> None:
    ...
    if total_comparisons != 96:
        raise RendererOutputError(f"portrait direct comparison aggregate mismatch: {total_comparisons}/96")
    print(f"aggregate portrait direct comparisons: {total_comparisons}/96")
    print("no-face 64x64 watermark-safe no-op comparisons: 8/8")
...
def run_self_tests() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        expect_error("ROI watermark", lambda: mouth_roi(100, 100), "not wholly above watermark")
...
def main() -> int:
    ...
    try:
        ...
        print(f"phase 39 mouth renderer output check failed: {error}", file=sys.stderr)
        return 1
    return 0
