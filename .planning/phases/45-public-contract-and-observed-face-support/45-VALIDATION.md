...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 45-01-01 | 01 | 1 | FACE-07, FACE-08, FACE-09, FACE-12 | T45-01 | finite positive-only scalar normalization | unit | `swift test --package-path BeautySDK --filter BeautyCoreTests.BeautyParametersTests` | ✅ extend existing | ⬜ pending |
| 45-01-02 | 01 | 1 | FACE-07, FACE-08, FACE-09, FACE-12 | T45-01 | 48-key compatibility, 52-key independence, preset neutrality | unit/resource | `swift test --package-path BeautySDK --filter BeautyCoreTests.BeautyParametersTests && swift test --package-path BeautySDK --filter BeautyResourcesTests.BeautyResourceCatalogTests` | ✅ extend existing | ⬜ pending |
| 45-02-01 | 02 | 1 | SUPP-01, SUPP-04 | T45-02 | request-scoped package-only Vision payload | unit | `swift test --package-path BeautySDK --filter BeautyDetectionTests.VisionFaceDetectorTests` | ✅ extend existing | ⬜ pending |
| 45-02-02 | 02 | 1 | SUPP-01, SUPP-02 | T45-02 | one-time mapping and reversal-only canonical direction | unit | `swift test --package-path BeautySDK --filter BeautyDetectionTests.FaceObservationMappingTests` | ✅ extend existing | ⬜ pending |
| 45-03-01 | 03 | 2 | SUPP-02 | T45-03 | bounded face-specific topology validation | unit/Wave 0 | `swift test --package-path BeautySDK --filter BeautyEffectsTests.BeautyFaceGeometryAdapterTests` | ✅ extend existing | ⬜ pending |
| 45-03-02 | 03 | 2 | SUPP-01, SUPP-02 | T45-03 | malformed observed support preserves legacy proxy and eligible siblings | integration/unit | `swift test --package-path BeautySDK --filter BeautyEffectsTests.BeautyFaceGeometryAdapterTests && swift test --package-path BeautySDK --filter BeautyDetectionTests` | ✅ extend existing | ⬜ pending |
| 45-04-01 | 04 | 3 | SUPP-04 | T45-04 | fail-closed public/privacy/dependency/resource/Demo classifier | self-test | `python3 .planning/phases/45-public-contract-and-observed-face-support/check_face_support_boundaries.py --self-test` | ❌ Wave 0 | ⬜ pending |
| 45-04-02 | 04 | 3 | FACE-07, FACE-08, FACE-09, FACE-12, SUPP-01, SUPP-02, SUPP-04 | T45-04 | synchronized owners and live boundary gate | package/boundary | `swift test --package-path BeautySDK && python3 .planning/phases/45-public-contract-and-observed-face-support/check_face_support_boundaries.py && git diff --check` | ❌ Wave 0 | ⬜ pending |
...
- [ ] `.planning/phases/45-public-contract-and-observed-face-support/check_face_support_boundaries.py` — fail-closed `rg` status classification, path containment, public/Codable/persistence/diagnostic/Demo/dependency/semantic-resource checks, fixed preset SHA-256 checks, and adversarial `--self-test`.
- [ ] `BeautySDK/Tests/BeautyEffectsTests/BeautyFaceGeometryAdapterTests.swift` — face-specific topology fixtures and exact threshold matrices created before or with adapter implementation; no opaque reuse of eye constants.
