...
| `眼睛` | partial | `BeautyEffects` | Four prior fields plus `eyeHeight`, `eyeLength`, `upperEyelidLift`, `pupilSize`, `gazeCorrection`, `lowerEyelidDrop`, signed `eyeTilt`, `innerCornerOpen`, `outerCornerOpen`, `eyeSymmetry`, and the implemented still-image `scleraRednessReduction` route; product-facing `祛红血丝` is `future` and unproven after the Plan 64-12 `gaps_found` candidate. | A fresh independent no-failure/no-skip candidate is required for `祛红血丝`; `去脂` remains future pending local retouch/segmentation design. | Phase 62/63 and Phase 64 output/safety/review implementation facts remain recorded, but product authorization is fully re-quarantined. SAFE-06 DeviceRGB/named-sRGB remains Phase 65-only. |
