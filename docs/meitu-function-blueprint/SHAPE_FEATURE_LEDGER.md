...
| `眼睛` | 祛红血丝 | future | The still-image `scleraRednessReduction` infrastructure remains present, but canonical `64-VERIFICATION.md: gaps_found` makes its Phase 64 product proof unproven; this evidentiary quarantine restores the D-17 pre-promotion state, aggregate `眼睛` remains `partial`, and `去脂` remains `future`. | A later independent eligibility artifact must satisfy D-18 before D-19 may reapply the exact promotion; the D-20 sibling boundary remains unchanged. |
