...
| `眼睛` | 祛红血丝 | implemented | Independent still-image `scleraRednessReduction`; Phase 62 evidence/admission, Phase 63 request-local per-eye guarded provider, and Phase 64 standalone public-facade output, adversarial safety, original-detail review, and exact promotion after the Plan 09 independent `eligible_promotion_pending` verdict. | Complete for the bounded SDK-core still-image slice after independent eligibility and is promotion-pending independent post-promotion candidate plus canonical final transaction; aggregate `眼睛` remains `partial` and `去脂` remains `future`. |
