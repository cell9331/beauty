...
| `眼睛` | 祛红血丝 | future | The still-image `scleraRednessReduction` implementation, public output, per-eye safety evidence, and original-detail review remain recorded facts, but the immutable Plan 64-12 candidate is `gaps_found`; product authorization is therefore unproven and fully re-quarantined. | A later fresh independent candidate must pass every mandatory command without failure or skip before the exact promotion can be reconsidered. Aggregate `眼睛` remains `partial`; `去脂` remains `future`; SAFE-06 DeviceRGB/named-sRGB remains Phase 65-only. |
