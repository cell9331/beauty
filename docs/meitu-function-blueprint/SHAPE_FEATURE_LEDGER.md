...
| `脸型` | 面部流畅 | implemented | Independent positive-only `faceContourSmooth`; Phase 45 public contract/observed contour support, Phase 46 named provider behavior, Phase 47 isolated public-facade output, and Phase 48 final cap, degradation, convergence, privacy, and boundary evidence. | Complete independently for observed contour continuity; no semantic-region behavior is implied. |
| `脸型` | 太阳穴 | implemented | Independent positive-only `templeFullness`; Phase 45 public contract/observed contour support, Phase 46 named provider behavior, Phase 47 isolated public-facade output, and Phase 48 final cap, degradation, convergence, privacy, and boundary evidence. | Complete independently for upper-lateral contour geometry only. |
| `脸型` | 颧骨 | implemented | Independent positive-only `cheekboneSlim`; Phase 45 public contract/observed contour support, Phase 46 named provider behavior, Phase 47 isolated public-facade output, and Phase 48 final cap, degradation, convergence, privacy, and boundary evidence. | Complete independently for mid-lateral contour geometry only. |
...
| `脸型` | 去双下巴 | future | None. | Needs an approved local semantic-region implementation and reproducible clean-clone fixtures; no cloud or remote AI by default. |
| `脸型` | 去双下巴 Pro | future | None. | Needs the same approved local semantic-region implementation and reproducible clean-clone fixtures; Pro/commercial gating remains out of scope. |
| `脸型` | 尖下巴 | implemented | Independent positive-only `chinTaper`; Phase 45 public contract/observed contour and centerline support, Phase 46 named provider behavior, Phase 47 isolated public-facade output, and Phase 48 final cap, degradation, convergence, privacy, and boundary evidence. | Complete independently for centerline-gated apex-neighbor taper only. |
...
| `脸型` | 发际线 | future | None. | Needs an approved local segmentation/resource implementation and reproducible clean-clone fixtures if promoted. |
...
| `眼睛` | 眼高 | implemented | Independent positive-only `eyeHeight`; Phase 41 contract/contour support, Phase 42 provider behavior, Phase 43 public output, and Phase 44 exact-cap, degradation, conflict, privacy, and boundary evidence. | Complete independently for contour-height geometry only. |
| `眼睛` | 长度 | implemented | Independent positive-only `eyeLength`; Phase 41 contract/contour support, Phase 42 provider behavior, Phase 43 public output, and Phase 44 exact-cap, degradation, conflict, privacy, and boundary evidence. | Complete independently for contour-length geometry only. |
...
| `眼睛` | 提肌 | implemented | Independent positive-only `upperEyelidLift`; Phase 41 contract/upper-contour support, Phase 42 provider behavior, Phase 43 public output, and Phase 44 exact-cap, degradation, conflict, privacy, and boundary evidence. | Complete independently for upper-lid geometry only. |
| `眼睛` | 眼瞳大小 | implemented | Independent positive-only `pupilSize`; Phase 41 contract/validated pupil support, Phase 42 provider behavior, Phase 43 public output, and Phase 44 exact-cap, degradation, conflict, privacy, and boundary evidence. | Missing/implausible pupil removes this field only; no raw pupil is public or diagnostic. |
| `眼睛` | 眼神矫正 | implemented | Independent positive-only `gazeCorrection`; Phase 41 contract/validated pupil support, Phase 42 bounded provider behavior, Phase 43 public output plus package aggregate, and Phase 44 exact-cap, degradation, conflict, privacy, and boundary evidence. | Package aggregate pupil-to-own-center reduction is authoritative; image-only mirror inference is not evidence. |
| `眼睛` | 眼睑下至 | implemented | Independent positive-only `lowerEyelidDrop`; Phase 41 contract/lower-contour support, Phase 42 provider behavior, Phase 43 public output, and Phase 44 exact-cap, degradation, conflict, privacy, and boundary evidence. | Complete independently for lower-lid geometry only. |
...
| `眼睛` | 倾斜 | implemented | Independent signed `eyeTilt`; Phase 41 contract/paired-contour support, Phase 42 positive/negative provider behavior, Phase 43 direct signed public output, and Phase 44 exact-cap, degradation, conflict, privacy, and boundary evidence. | Both directions preserve sign through one retained conflict scale. |
| `眼睛` | 祛红血丝 | implemented | Bounded SDK-core still-image `BeautyParameters.scleraRednessReduction` through public `BeautyEngine.apply`: Phase 62 independent evidence/admission, Phase 63 guarded per-eye production, and Phase 64 public output plus bilateral protected-region adversarial proof and fresh 637/0/0/8 no-skip authority. Current authority is `64-TERMINAL-R2-SCLERA-OUTPUT-EVIDENCE.md`, `64-TERMINAL-R2-REVIEW.md`, `64-TERMINAL-R2-CODE-REVIEW.md`, `64-TERMINAL-R2-REVIEW-FIX.md`, `64-TERMINAL-R2-SECURITY.md`, and `64-TERMINAL-R2-PRE-PROMOTION-VERIFICATION.md`. | Promotion pending independent candidate/final verification. Aggregate `眼睛` remains `partial`; `去脂` remains `future`; the Demo row remains disabled; SAFE-06 DeviceRGB/named-sRGB remains Phase 65-only. |
| `眼睛` | 内眼角 | implemented | Independent positive-only `innerCornerOpen`; Phase 41 contract/inner-corner support, Phase 42 provider behavior, Phase 43 distinct public output, and Phase 44 exact-cap, degradation, conflict, privacy, and boundary evidence. | Complete independently; does not borrow outer-corner or tail-lift evidence. |
| `眼睛` | 外眼角 | implemented | Independent positive-only `outerCornerOpen`; Phase 41 contract/outer-corner support, Phase 42 provider behavior, Phase 43 distinct public output, and Phase 44 exact-cap, degradation, conflict, privacy, and boundary evidence. | Complete independently; does not borrow inner-corner or tail-lift evidence. |
| `眼睛` | 对称 | implemented | Independent positive-only `eyeSymmetry`; Phase 41 paired-contour contract, Phase 42 bounded midpoint provider behavior, Phase 43 distinct public output, and Phase 44 exact-cap, degradation, conflict, privacy, and boundary evidence. | Exact neutral dead zone and maximum blend remain bounded; no public geometry is exposed. |
...
| `嘴唇` | 白牙 | implemented | Independent still-image `teethWhitening`; Phase 59 rights-approved evidence/admission, Phase 60 request-local provider and immutable-source integration, and Phase 61 strict public-facade output, adversarial safety, and original-detail review. | Complete for the bounded SDK-core still-image slice; Demo activation, realtime/pixel-buffer support, population/device/performance/commercial/release claims, `祛红血丝`, and `去脂` remain outside this row. |
...
| `眉毛` | 上下 | implemented | Independent signed `eyebrowYPosition`; Phase 49 public contract and observed eyebrow support, Phase 50 named provider/pipeline behavior, Phase 51 positive/negative public-facade output, and Phase 52 final cap, lifecycle, convergence, privacy, and boundary evidence. | Complete at SDK-core scope for bilateral vertical translation only. |
| `眉毛` | 粗细 | implemented | Independent signed `eyebrowThickness`; Phase 49 public contract and observed eyebrow support, Phase 50 trace-normal provider behavior, Phase 51 positive/negative public-facade output, and Phase 52 final cap, local-failure, convergence, privacy, and boundary evidence. | Complete at SDK-core scope for geometry-only trace thickness; no makeup or texture synthesis is implied. |
| `眉毛` | 长短 | implemented | Independent signed `eyebrowLength`; Phase 49 public contract and observed eyebrow support, Phase 50 outer-endpoint provider behavior, Phase 51 positive/negative public-facade output, and Phase 52 final cap, local-failure, convergence, privacy, and boundary evidence. | Complete at SDK-core scope for outer-endpoint length geometry only. |
| `眉毛` | 间距 | implemented | Independent signed `eyebrowSpacing`; Phase 49 public contract and paired observed eyebrow support, Phase 50 bilateral whole-brow provider behavior, Phase 51 positive/negative public-facade output, and Phase 52 final cap, pair-failure, convergence, privacy, and boundary evidence. | Complete at SDK-core scope for paired whole-brow spacing only. |
| `眉毛` | 眉头间距 | implemented | Independent signed `eyebrowHeadSpacing`; Phase 49 public contract and canonical inner-head support, Phase 50 inner-endpoint provider behavior, Phase 51 positive/negative public-facade output distinct from whole-brow spacing, and Phase 52 final cap, local-failure, convergence, privacy, and boundary evidence. | Complete independently at SDK-core scope; this row does not borrow whole-brow spacing evidence. |
| `眉毛` | 倾斜 | implemented | Independent signed `eyebrowTilt`; Phase 49 public contract and canonical observed eyebrow support, Phase 50 local-rotation provider behavior, Phase 51 positive/negative public-facade output, and Phase 52 final cap, lifecycle, convergence, privacy, and boundary evidence. | Complete at SDK-core scope with signed direction preserved across the retained conflict scale. |
| `眉毛` | 眉峰 | implemented | Independent positive-only `eyebrowPeakDefinition`; Phase 49 public contract and validated interior-apex support, Phase 50 bounded apex provider behavior, Phase 51 distinct public-facade peak output, and Phase 52 final cap, apex-failure, convergence, privacy, and boundary evidence. | Complete independently at SDK-core scope; no uniform thickness, makeup, or texture behavior is implied. |
