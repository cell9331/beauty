...
| `眼睛` | 祛红血丝 | implemented | Bounded SDK-core opaque still-image `BeautyParameters.scleraRednessReduction` through public `BeautyEngine`: Phase 62 independent evidence/admission, Phase 63 guarded per-eye production, Phase 64 canonical output/protected-region promotion, and Phase 65 combined/failure/privacy/named-sRGB closeout. | Complete for this bounded SDK-core still-image row. Aggregate `眼睛` remains `partial`; `去脂` remains `future`; the Demo row remains disabled and nil-mapped; realtime/pixel-buffer, device/performance, commercial, packaging, shipping, launch and release-readiness claims remain excluded. |
