...
| `眼睛` | 祛红血丝 | implemented | Independent still-image `scleraRednessReduction`; Phase 62 rights-approved evidence/admission, Phase 63 actual-support guarded per-eye provider/integration, and Phase 64 strict public-facade output, color-independent/recolored-protected safety, original-detail review, and exact promotion. | Complete for the bounded SDK-core still-image slice; Demo activation, realtime/pixel-buffer, population/device/performance/commercial/release claims and `去脂` remain outside this row. |
