...
| Beauty shaping | 鼻子 | partial | `BeautyEffects` | `BeautyDetection` nose landmarks, `BeautyRender` unified warp | `noseSlim`, `noseWingSlim`, signed `noseTipSize`, `noseBridge`; implemented subtools: 大小, 鼻翼, 鼻梁, 鼻尖. | `提升`, `山根` alias/parameter decision, and additional controls remain future. | Phase 31 public-facade 196/196 and 30/30 output evidence plus Phase 32 safety/degradation/boundary evidence cover exactly four rows. | Uses SDK domain `nose`; branch remains partial. |
