...
- EVID-01 through EVID-05 and LID-01 implement D-01/D-04/D-09 through D-16 as an offline, one-feature-at-a-time evidence decision boundary. A feature becomes eligible only from a complete rights-approved genuine positive/negative original-mask-after bundle whose frozen review set is accepted at original detail; mechanics, synthetic, AI-generated, historical, parked, disabled, incomplete, or rejected rows have zero product weight.
- Current decisions are independently and honestly closed: teeth is `missing_genuine_positive`; sclera is `missing_genuine_positive` plus `incomplete_asset_triple`; upper-eyelid fullness is `missing_genuine_positive` plus `non_warp_design_unqualified`. Each row has zero eligible/reviewed/accepted/rejected counts and zero naturalness weight. One sibling can open later without promoting another.
