...
Current repository state as of 2026-07-13 after Phase 32 nose safety, ledger, and documentation closeout:
...
| GSD planning | 4 | v1.7 requirements and Phases 31-32 trace nine requirements to public-facade output, safety/degradation, exact four-row promotion, and current-owner closeout. Both phases have Nyquist validation. | Run milestone audit and archive after the final evidence pass. |
| SDK Package | 4 | `BeautySDK` retains the existing package/facade structure and 31-field public inventory. Phase 32 full `swift test --package-path BeautySDK` passes after signed-tip and fail-closed nose fixes. | Optimized profiling, physical-device checks, and packaging remain future gates. |
...
| Tests | 4 | Phase 32 focused suites cover nose normalization, exact caps, signed direction, missing/stale zeroing, reused `0.5`, safe no-face continuation, combined weakening, facade output, and renderer regression. | Add tests only for separately scoped future tools; retain device/Demo gaps explicitly. |
| Security | 4 | Phase 32 fail-closed scans cover raw geometry, forbidden imports, network/cloud, commercial paths, public inventory, dependencies, and tracked generated artifacts; review is clean with zero open threats. | Reopen manifest, dependency, network, resource-integrity, and raw-geometry review when behavior changes or packaging starts. |
| Reliability | 4 | Nose missing/stale paths zero all four effective strengths, reused geometry stays at `0.5`, safe domains continue, and diagnostics remain redacted/aggregate. | Keep the 600-second preview, physical iPhone, screenshot, and optimized profiling routes future. |
| Product acceptance | 4 | Phases 31-32 prove 196/196 outputs, 30/30 portrait comparisons, both signed tip directions, and exactly `大小`, `鼻翼`, `鼻梁`, `鼻尖`; branch-level `鼻子` remains `partial`, with `山根`/`提升` future. | Do not infer whole-branch, device, commercial, packaging, or launch readiness. |
