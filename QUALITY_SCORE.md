...
Current repository state as of 2026-07-13 after Phase 30 eye safety, ledger, and documentation closeout:
...
| GSD planning | 4 | `.planning/PROJECT.md`, `.planning/STATE.md`, `.planning/ROADMAP.md`, and the v1.6 milestone archives record all nine v1.6 requirements complete, 9/9 integration checks, 5/5 flows, and both phases Nyquist compliant. Exactly four evidence-backed eye rows are implemented while branch-level `眼睛` remains `partial`. | Start a fresh milestone with new requirements; keep future eye tools separately scoped. |
| SDK Package | 4 | `BeautySDK/Package.swift` exists with `BeautyCore`, `BeautyDetection`, `BeautyRender`, `BeautyEffects`, `BeautyResources`, facade `BeautySDK`, and `BeautyExampleRenderer`. Phase 30 `swift test --package-path BeautySDK` passed with 178 XCTest cases after eye safety and degradation closeout. | Optimized profiling, physical-device checks, and packaging remain future gates. |
...
| Tests | 4 | Phase 30 focused suites cover public eye normalization, exact caps, missing/reused/stale eye degradation, no-face behavior, combined weakening, and renderer regression. The full SDK suite passed with 178 tests; Phase 25 Demo camera/privacy/import evidence remains the latest Demo xcodebuild pass. | Add tests only for separately scoped future eye tools or other new behavior; retain physical-device and Demo evidence gaps explicitly. |
| Security | 4 | Phase 30 active-source scans found zero public/SPI raw-geometry candidates, forbidden Demo/renderer internal imports, network/cloud paths, commercial execution paths, and tracked generated artifacts; review is clean and security has zero open threats. Phase 25 privacy-manifest deferral and bundled-resource trust evidence remain current. | Reopen manifest, dependency, network, resource-integrity, and raw-geometry review when behavior changes or packaging starts. |
| Reliability | 4 | Existing tests cover typed errors, reset, backpressure, stale work, degradation, resource failures, and non-mutating JSON import failures. Phase 30 adds missing-eye, reused-eye, stale-eye, public no-face, and combined-geometry weakening evidence while safe unrelated domains continue. | Keep the 600-second preview route, physical iPhone checks, screenshot reruns, and optimized profiling as explicit future evidence. |
| Product acceptance | 4 | Phases 29 and 30 prove the existing-parameter eye slice with 161/161 renderer outputs, 36/36 portrait comparisons, conservative safety/degradation behavior, and exactly four implemented rows: `大小`, `上下`, `眼距`, and `眼尾上扬`. Branch-level `眼睛` remains `partial`. | Scope future eye tools, screenshot reruns, hardware checks, external resource packages, and packaging separately; do not infer whole-branch or release readiness. |
