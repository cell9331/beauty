...
| Tests | 4 | Focused Full/composition/adversarial/integration/Focal suites pass 60/60. Default full SwiftPM passes 650 tests with eight documented opt-in skips; the current `scripts/run-no-skip-swiftpm.sh` gate passes the same 650 tests with all 8 opt-ins executed and zero skips/failures. | Keep the no-skip gate and colored-interior, sparse-trigger, pre-allocation budget, and strict area-ceiling regressions mandatory. |
